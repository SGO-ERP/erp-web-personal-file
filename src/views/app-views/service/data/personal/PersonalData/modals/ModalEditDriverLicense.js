...
const ModalEditDriverLicense = ({ isOpen, onClose, DriverLicenseDocument, source }) => {
    ...
    useEffect(() => {
        form.resetFields();
        if(DriverLicenseDocument?.document_link && isOpen===true) {
            const getFile = async () => {
                const file = await FileUploaderService.getFileByLink(DriverLicenseDocument.document_link);
                form.setFieldsValue({
                    dragger: [file],
                });
            };
    ...
    const onOk = async () => {
        try {
            ...
            const response = filesChanged ? await handleFileUpload(values.dragger) : DriverLicenseDocument.document_link;
            if (response) {
                const newObject = {
                    document_link: response,
                    id: DriverLicenseDocument?.id,
                };
        ...
        if (source === 'get') {
            ...
            dispatch(
                deleteByPath({
                    ...
                    document_link: DriverLicenseDocument.document_link,
                }),
            );
            // Add to Edited slice
            dispatch(
    ...
    const onDelete = () => {
        changeDispatchValues({ delete: true, id:DriverLicenseDocument.id });
    };
    const onCancel = () => {
        form.resetFields();
        ...
        <div onClick={(e) => e.stopPropagation()}>
            <Modal
                title={
                    ...
                    >
                        <span>
                            <IntlMessage id='edit.driving.license' />
                        </span>
                    </div>
                }
                open={isOpen}
...
export default ModalEditDriverLicense;
