...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminPenaltySlice";
import IntlMessage from "../../../../../../components/util-components/IntlMessage";
import { useAppDispatch, useAppSelector } from "../../../../../../hooks/useStore";
import { nameSorter, tablePenaltyDelete } from "../utils/TableFunctions";

interface Props {
    setRecord: (record: any) => void;
    record: any;
}
const RecoveryTable = ({ setRecord, record }: Props) => {
    const [currentPage, setCurrentPage] = useState<number>(0);
    const [currentPageSize, setCurrentPageSize] = useState<number>(0);

    const { penalty_types, tablePagination } = useAppSelector((state) => state.adminPenalties);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const filteredData =
        tableSearch.trim() === ""
            ? penalty_types.data.objects
            : penalty_types.data.objects.filter(
                  (e: any) =>
