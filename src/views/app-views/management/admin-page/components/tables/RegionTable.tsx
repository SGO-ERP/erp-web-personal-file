...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminRegionSlice";
import { tableRegionDelete, nameSorter } from "../utils/TableFunctions";
...
const RegionTable = ({ setRecord, record }: Props) => {
    ...
    const { region_types, tablePagination } = useAppSelector((state) => state.adminRegion);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(region_types.loading);
    }, [region_types.loading]);
