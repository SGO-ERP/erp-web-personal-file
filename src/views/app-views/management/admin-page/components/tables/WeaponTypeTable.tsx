...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminWeaponSlice";
import { nameSorter, tableViolationDelete } from "../utils/TableFunctions";
...
const WeaponTypeTable = ({ setRecord, record }: Props) => {
    ...
    const { weapon_types, tablePagination } = useAppSelector((state) => state.adminWeaponType);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(weapon_types.loading);
    }, [weapon_types.loading]);
    ...
    useEffect(() => {
        if (!weapon_types.data.objects) return;

        const filteredData =
            tableSearch.trim() === ""
                ? weapon_types.data?.objects
                : weapon_types.data.objects.filter(
                      (e: any) =>
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableViolationDelete(record.id);
                            }
                        }}
                    >
...
export default WeaponTypeTable;
