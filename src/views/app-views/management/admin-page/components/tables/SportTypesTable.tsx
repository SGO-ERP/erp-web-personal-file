...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminSportTypesSlice";
import IntlMessage from "../../../../../../components/util-components/IntlMessage";
import { useAppDispatch, useAppSelector } from "../../../../../../hooks/useStore";
import { nameSorter, tableSportDelete } from "../utils/TableFunctions";

interface Props {
    setRecord: (record: any) => void;
    record: any;
}
const SportTypesTable = ({ setRecord, record }: Props) => {
    const [currentPage, setCurrentPage] = useState<number>(0);
    const [currentPageSize, setCurrentPageSize] = useState<number>(0);

    const { sport_types, tablePagination } = useAppSelector((state) => state.adminSportTypes);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const filteredData =
        tableSearch.trim() === ""
            ? sport_types.data.objects
            : sport_types.data.objects.filter(
                  (e: any) =>
...
export default SportTypesTable;
