...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminCitizenshipSlice";
import { nameSorter, tableCitizenshipDelete } from "../utils/TableFunctions";
...
const CitizenshipTable = ({ setRecord, record }: Props) => {
    ...
    const { citizenship_types, tablePagination } = useAppSelector(
        (state) => state.adminCitizenship,
    );
    ...
    useEffect(() => {
        setLoading(citizenship_types.loading);
    }, [citizenship_types.loading]);
    ...
    useEffect(() => {
        const filteredData =
            tableSearch.trim() === ""
                ? citizenship_types.data
                : citizenship_types.data.filter(
                      (e: any) =>
    ...
    }, [citizenship_types.data, tableSearch]);
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableCitizenshipDelete(record.id);
                            }
                        }}
                    >
