...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminClothingModelsSlice";
import { nameSorter, tableClothingModelDelete } from "../utils/TableFunctions";
...
const ClothingModelsTable = ({ setRecord, record }: Props) => {
    ...
    const { clothing_models, tablePagination } = useAppSelector(
        (state) => state.adminClothingModels,
    );
    ...
    useEffect(() => {
        setLoading(clothing_models.loading);
    }, [clothing_models.loading]);
    ...
    useEffect(() => {
        if (!clothing_models.data) return;

        const filteredData =
            tableSearch.trim() === ""
                ? clothing_models.data
                : clothing_models.data.filter(
                      (e: any) =>
    ...
    }, [clothing_models.data, tableSearch]);
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableClothingModelDelete(record.id);
                            }
                        }}
                    >
...
export default ClothingModelsTable;
