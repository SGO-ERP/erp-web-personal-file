...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminLanguagesSlice";
import IntlMessage from "../../../../../../components/util-components/IntlMessage";
import { useAppDispatch, useAppSelector } from "../../../../../../hooks/useStore";
import { nameSorter, tableLanguageDelete } from "../utils/TableFunctions";

interface Props {
    setRecord: (record: any) => void;
    record: any;
}
const LanguageTable = ({ setRecord, record }: Props) => {
    ...
    const { language_types, tablePagination } = useAppSelector((state) => state.adminLanguages);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(language_types.loading);
    }, [language_types.loading]);
    ...
    useEffect(() => {
        const filteredData =
            tableSearch.trim() === ""
                ? language_types.data.objects
                : language_types.data.objects.filter(
                      (e: any) =>
    ...
    }, [language_types.data, tableSearch]);
