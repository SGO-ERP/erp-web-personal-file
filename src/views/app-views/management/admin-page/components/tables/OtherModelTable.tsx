...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminOtherModelSlice";
import { nameSorter, tableOtherModelDelete } from "../utils/TableFunctions";
...
const OtherModelTable = ({ setRecord, record }: Props) => {
    ...
    const { other_models, tablePagination } = useAppSelector((state) => state.adminOtherModel);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(other_models.loading);
    }, [other_models.loading]);
    ...
    useEffect(() => {
        if (!other_models.data) return;

        const filteredData =
            tableSearch.trim() === ""
                ? other_models.data
                : other_models.data.filter(
                      (e: any) =>
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableOtherModelDelete(record.id);
                            }
                        }}
                    >
...
export default OtherModelTable;
