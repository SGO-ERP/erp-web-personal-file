...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminAcademicTitleSlice";
import IntlMessage from "../../../../../../components/util-components/IntlMessage";
import { useAppDispatch, useAppSelector } from "../../../../../../hooks/useStore";
import { nameSorter, tableAcademicTitleDelete } from "../utils/TableFunctions";
...
const AcademicTitleDegreesTable = ({ setRecord, record }: Props) => {
    const [currentPage, setCurrentPage] = useState<number>(0);
    const [currentPageSize, setCurrentPageSize] = useState<number>(0);

    const { academic_title_types, tablePagination } = useAppSelector(
        (state) => state.adminAcademicTitles,
    );
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const filteredData =
        tableSearch.trim() === ""
            ? academic_title_types.data.objects
            : academic_title_types.data.objects.filter(
                  (e: any) =>
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableAcademicTitleDelete(record.id);
                            }
                        }}
                    >
    ...
    if (academic_title_types.loading) {
    ...
    return (
        <React.Fragment>
            <Table
                ...
                loading={academic_title_types.loading}
                pagination={{
                    current: currentPage,
                    defaultPageSize: 10,
...
export default AcademicTitleDegreesTable;
