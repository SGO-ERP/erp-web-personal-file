...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminScienceSlice";
import { nameSorter, tableScienceDelete } from "../utils/TableFunctions";
...
const ScienceTable = ({ setRecord, record }: Props) => {
    ...
    const { science_types, tablePagination } = useAppSelector((state) => state.adminSciences);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(science_types.loading);
    }, [science_types.loading]);
    ...
    useEffect(() => {
        const filteredData =
            tableSearch.trim() === ""
                ? science_types.data.objects
                : science_types.data.objects.filter(
                      (e: any) =>
    ...
    }, [science_types.data, tableSearch]);
