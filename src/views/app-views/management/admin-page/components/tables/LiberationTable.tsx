...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminLiberationSlice";
import { nameSorter, tableLiberationDelete } from "../utils/TableFunctions";
...
const LiberationTable = ({ setRecord, record }: Props) => {
    ...
    const { liberation_types, tablePagination } = useAppSelector((state) => state.adminLiberation);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(liberation_types.loading);
    }, [liberation_types.loading]);
    ...
    useEffect(() => {
        const filteredData =
            tableSearch.trim() === ""
                ? liberation_types.data.objects
                : liberation_types.data.objects.filter(
                      (e: any) =>
    ...
    }, [liberation_types.data, tableSearch]);
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableLiberationDelete(record.id);
                            }
                        }}
                    >
...
export default LiberationTable;
