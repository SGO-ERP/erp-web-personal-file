...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminOtherTypeSlice";
...
const OtherTypesTable = ({ setRecord, record }: Props) => {
    ...
    const { other_types, tablePagination } = useAppSelector((state) => state.adminOtherType);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(other_types.loading);
    }, [other_types.loading]);
    ...
    useEffect(() => {
        if (!other_types.data.objects) return;

        const filteredData =
            tableSearch.trim() === ""
                ? other_types.data.objects
                : other_types.data.objects.filter(
                      (e: any) =>
...
export default OtherTypesTable;
