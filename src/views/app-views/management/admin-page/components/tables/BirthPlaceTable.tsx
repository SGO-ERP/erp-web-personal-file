...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminBirthPlaceSlice";
import IntlMessage from "../../../../../../components/util-components/IntlMessage";
import { useAppDispatch, useAppSelector } from "../../../../../../hooks/useStore";
import { nameSorter, tableBirthPlaceDelete } from "../utils/TableFunctions";
...
const BirthPlaceTable = ({ setRecord, record }: Props) => {
    ...
    const { birthPlace_types, tablePagination } = useAppSelector((state) => state.adminBirthPlaces);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(birthPlace_types.loading);
    }, [birthPlace_types.loading]);
    ...
    useEffect(() => {
        const filteredData =
            tableSearch.trim() === ""
                ? birthPlace_types.data
                : birthPlace_types.data.filter(
                      (e: any) =>
    ...
    }, [birthPlace_types.data, tableSearch]);
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableBirthPlaceDelete(record.id);
                            }
                        }}
                    >
...
export default BirthPlaceTable;
