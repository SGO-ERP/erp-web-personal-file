...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminViolationSlice";
...
const ViolationTable = ({ setRecord, record }: Props) => {
    ...
    const { violation_types, tablePagination } = useAppSelector((state) => state.adminViolation);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(violation_types.loading);
    }, [violation_types.loading]);
    ...
    }, [violation_types.data, tableSearch]);
