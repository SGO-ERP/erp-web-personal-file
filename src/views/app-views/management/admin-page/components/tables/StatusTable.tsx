...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminStatusesSlice";
import IntlMessage from "../../../../../../components/util-components/IntlMessage";
import { useAppDispatch, useAppSelector } from "../../../../../../hooks/useStore";
import { nameSorter, tableStatusDelete } from "../utils/TableFunctions";
...
const StatusTable = ({ setRecord, record }: Props) => {
    const [currentPage, setCurrentPage] = useState<number>(0);
    const [currentPageSize, setCurrentPageSize] = useState<number>(0);

    const { status_types, tablePagination } = useAppSelector((state) => state.adminStatuses);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const filteredData =
        tableSearch.trim() === ""
            ? status_types.data.objects
            : status_types.data.objects.filter(
                  (e: any) =>
