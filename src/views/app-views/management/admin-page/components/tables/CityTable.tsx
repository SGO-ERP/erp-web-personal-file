...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminCitySlice";
import { tableCityDelete, nameSorter } from "../utils/TableFunctions";
...
const CityTable = ({ setRecord, record }: Props) => {
    ...
    const { city_types, tablePagination } = useAppSelector((state) => state.adminCity);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(city_types.loading);
    }, [city_types.loading]);
