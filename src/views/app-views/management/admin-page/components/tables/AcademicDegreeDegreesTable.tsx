...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminAcademicDegreeSlice";
import IntlMessage from "../../../../../../components/util-components/IntlMessage";
import { useAppDispatch, useAppSelector } from "../../../../../../hooks/useStore";
import { nameSorter, tableAcademicDegreeDelete } from "../utils/TableFunctions";

interface Props {
    setRecord: (record: any) => void;
    record: any;
}
const AcademicDegreeDegreesTable = ({ setRecord, record }: Props) => {
    const [currentPage, setCurrentPage] = useState<number>(0);
    const [currentPageSize, setCurrentPageSize] = useState<number>(0);

    const { academic_degree_types, tablePagination } = useAppSelector(
        (state) => state.adminAcademicDegrees,
    );
    const { tableSearch } = useAppSelector((state) => state.adminPage);
    const filteredData =
        tableSearch.trim() === ""
            ? academic_degree_types.data.objects
            : academic_degree_types.data.objects.filter(
                  (e: any) =>
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableAcademicDegreeDelete(record.id);
                            }
                        }}
                    >
    ...
    if (academic_degree_types.loading) {
    ...
    return (
        <React.Fragment>
            <Table
                ...
                loading={academic_degree_types.loading}
                pagination={{
                    current: currentPage,
                    defaultPageSize: 10,
...
export default AcademicDegreeDegreesTable;
