...
import { nameSorter, tableInstitutionDegreeDelete } from "../utils/TableFunctions";
import { setTablePage, setTableSize } from "store/slices/admin-page/adminInstitutionDegreeSlice";
...
const InstitutionDegreeTypesTable = ({ setRecord, record }: Props) => {
    ...
    const { institution_degree_types, tablePagination } = useAppSelector(
        (state) => state.adminInstitutionDegrees,
    );
    ...
    useEffect(() => {
        setLoading(institution_degree_types.loading);
    }, [institution_degree_types.loading]);
    ...
    useEffect(() => {
        const filteredData =
            tableSearch.trim() === ""
                ? institution_degree_types.data.objects
                : institution_degree_types.data.objects.filter(
                      (e: any) =>
    ...
    }, [institution_degree_types.data, tableSearch]);
            ...
            render: (_: string, record: any) => (
                <Row>
                    <Button
                        ...
                        onClick={() => {
                            if (record.id) {
                                tableInstitutionDegreeDelete(record.id);
                            }
                        }}
                    >
...
export default InstitutionDegreeTypesTable;
