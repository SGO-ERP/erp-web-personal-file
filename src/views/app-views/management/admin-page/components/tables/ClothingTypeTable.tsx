...
import { setTablePage, setTableSize } from "store/slices/admin-page/adminClothingTypeSlice";
...
const ClothingTypesTable = ({ setRecord, record }: Props) => {
    ...
    const { clothing_types, tablePagination } = useAppSelector((state) => state.adminClothingType);
    const { tableSearch } = useAppSelector((state) => state.adminPage);

    const dispatch = useAppDispatch();

    useEffect(() => {
        setLoading(clothing_types.loading);
    }, [clothing_types.loading]);
    ...
    useEffect(() => {
        if (!clothing_types.data.objects) return;

        const filteredData =
            tableSearch.trim() === ""
                ? clothing_types.data.objects
                : clothing_types.data.objects.filter(
                      (e: any) =>
    ...
    }, [clothing_types.data, tableSearch]);
